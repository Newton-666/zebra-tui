...
**② 身份（每次进群注入）**：每个成员有一个**一句话身份/职责**（`members[].role`，在手动向导里
逐个成员输入，或由「一句话建队」生成），持久化在 `team.json`。注入策略：

| 场景 | 是否注入身份 | 原因 |
|---|---|---|
| 新建团队 | 是 | 上下文是新的 |
| 引擎被重建后 resume | 是（全员） | agent 进程换了，上下文丢失 |
| 某个窗格崩溃被自动复活 | 是（该成员） | 新进程没有上下文 |
| resume 且窗格还活着 | 否 | 它的上下文里已经有身份，重复注入会白烧一轮 token |

需要强制重发用 `:brief`（同时补发身份与完整简报）。

**③ 团队简报（只发一次）**：更完整的那份（队友名册、协作命令、纪律）只在成员首次进群时注入，
全文写在 `sessions/<id>/BRIEF.md`。身份只一两句话、每次注入；简报较长、只发一次——这样开局成本很低。
...
- **v0.3.1** — 逐成员身份（手动向导新增「一句话身份/职责」步骤，可回车跳过）、身份注入策略（新建/重建/复活才注入，活窗格 resume 不重复）、修复引擎重建时 config 副本导致 paneIds 与启动命令被写坏的隐藏 bug
